...
exports.inscription = async (req, res) => {
  ...
  return { status: 201, json: () => res.status(201).json({ message: 'Inscription réussie.' }) };
};
...
exports.connexion = async (req, res) => {
  ...
  res.status(200).json({ message: 'Connexion réussie.', token, user: { prenom: user.prenom, nom: user.nom, email: user.email, _id: user._id } });
};
...
// ... reste du code inchangé (forgotPassword, resetPassword)